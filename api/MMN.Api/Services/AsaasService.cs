...
using MMN.Api.Models.Response.Asaas.Customer;
...
namespace MMN.Api.Services
{
    public interface IAsaasService
    {
        ...
        Task<List<PaymentResponse>> GetBoletosLast30DaysAsync();
    ...
    public class AsaasService : IAsaasService
    {
        ...
        // Listar boletos dos últimos 30 dias v3/payments?dueDate[ge]&dueDate[le]
        public async Task<List<PaymentResponse>> GetBoletosLast30DaysAsync()
        {
            var allPayments = new List<PaymentResponse>();
            var limit = 100;
            var offset = 0;
            ...
            while (true)
            {
                var url = $"{_baseUrl}v3/payments?limit={limit}&offset={offset}&dueDate[ge]={startDate}&dueDate[le]={endDate}";
                var request = new HttpRequestMessage(HttpMethod.Get, url);
                request.Headers.Add("User-Agent", "QuantaShop");
                request.Headers.Add("access_token", _apiKey);
                ...
                if (response.IsSuccessStatusCode)
                {
                    var list = JsonConvert.DeserializeObject<PaymentListResponse>(responseContent);
                    if (list?.Data == null || list.Data.Count == 0)
                    {
                        break;
                    }

                    allPayments.AddRange(list.Data);

                    if (allPayments.Count >= list.TotalCount)
                    {
                        break;
                    }
                ...
                {
                    var error = JsonConvert.DeserializeObject<BadRequestResponse>(responseContent);
                    throw new Exception($"Erro ao listar boletos: {string.Join(", ", error.Errors.Select(e => e.Description))}");
                }
            }
