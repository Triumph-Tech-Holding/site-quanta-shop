...
const handleSubmit = () => {
  if (!searchText.value && !productType.value) {
    ...
    router.push(
      `/busca?searchText=${searchText.value}&productType=${productType.value}`
    );
  } else if (searchText.value && !productType.value) {
    router.push(`/busca?searchText=${searchText.value}`);
  } else if (!searchText.value && productType.value) {
    router.push(`/busca?productType=${productType.value}`);
  } else {
    router.push(`/busca`);
  }
};
