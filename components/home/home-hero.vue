...
const DEFAULT_BANNERS: Banner[] = [
  { badge: 'Milhares de usuários economizando', title: 'Seu dinheiro <highlight>volta</highlight> a cada compra', sub: 'Compre nas suas lojas favoritas e receba cashback de verdade. Simples, transparente e instantâneo.', cta: 'Criar Conta Grátis', href: '/agencia/cadastro' },
...
.qs-hero .qs-hero__grid { position: relative; z-index: 2; display: grid; grid-template-columns: 1.05fr .95fr; gap: 44px; align-items: center; padding: 92px 0 80px; min-height: calc(100svh - 72px); }
...
.qs-hero__title { font-family: 'Jost','Inter',sans-serif; color: #fff; font-size: clamp(32px, 5.4vw, 62px); font-weight: 800; line-height: 1.07; letter-spacing: -.025em; margin: 12px 0 12px; min-height: 2.1em; }
...
.qs-hero__composer-label { margin-top: 18px; font-family: 'Kiye Sans','Inter','Jost',sans-serif; font-size: 13px; color: #bcd6da; }
...
.qs-hero__chips { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
...
.qs-hero__chat-body { display: flex; flex-direction: column; gap: 10px; padding: 16px 6px 6px; min-height: 296px; max-height: 360px; overflow: hidden; }
...
@media (max-width: 980px) {
  .qs-hero .qs-hero__grid { display: flex; flex-direction: column; gap: 20px; padding: 44px 0 60px; min-height: auto; }
