<template>
  <div>
    <div class="ag-page-header"><h1>Aniversariantes</h1><p>Usuários aniversariantes</p></div>
...
function ver(item: AniversarianteAdmin) { console.log('ver', item); }
onMounted(async () => {
  agenciaStore.loadFromStorage();
  if (!agenciaStore.isAdmin) { navigateTo('/agencia/painel'); return; }
  try {
    const { data } = await api.get('/admin/usuarios/aniversariantes', authHeader());
