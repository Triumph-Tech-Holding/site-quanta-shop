<template>
  <div>
    <div class="ag-page-header"><h1>Material de Apoio (Admin)</h1><p>Gerenciar materiais de apoio</p></div>
...
function ver(item: MaterialApoioAdmin) { console.log('ver', item); }
onMounted(async () => {
  agenciaStore.loadFromStorage();
  if (!agenciaStore.isAdmin) { navigateTo('/agencia/painel'); return; }
  try {
    const { data } = await api.get('/admin/materialApoio/listar', authHeader());
