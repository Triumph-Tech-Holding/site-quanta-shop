<template>
  <div>
    <div class="ag-page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
      <div><h1>Gerenciar Grupos</h1><p>Grupos de usuários e permissões</p></div>
      <button class="btn btn-ag-primary" @click="abrirNovo">+ Novo Grupo</button>
    </div>
    <div v-if="loading" class="ag-loading"><div class="spinner-border" /></div>
    <div v-else class="ag-card">
      <div v-if="itens.length === 0" class="ag-empty-state"><h5>Nenhum grupo encontrado</h5></div>
    ...
    <div v-if="showModal" class="ag-modal-overlay" @click.self="fecharModal">
      <div class="ag-modal">
        <div class="ag-modal-header"><h5 class="mb-0">{{ form.id ? 'Editar Grupo' : 'Novo Grupo' }}</h5><button class="btn-close" @click="fecharModal" /></div>
    ...
    <div v-if="showConfirm" class="ag-modal-overlay" @click.self="showConfirm = false">
      <div class="ag-modal" style="max-width:420px">
        ...
        <div class="ag-modal-body"><p>Excluir o grupo <strong>{{ itemParaExcluir?.nome }}</strong>?</p></div>
        <div class="ag-modal-footer">
