<template>
  <div>
    <div class="ag-page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
      <div><h1>Categorias</h1><p>Gerenciar categorias de lojas</p></div>
      <button class="btn btn-ag-primary" @click="abrirNovo">+ Nova Categoria</button>
    </div>
    <div v-if="loading" class="ag-loading"><div class="spinner-border" /></div>
    <div v-else class="ag-card">
      <div v-if="itens.length === 0" class="ag-empty-state"><h5>Nenhuma categoria encontrada</h5></div>
      <div v-else class="table-responsive">
        <table class="table ag-table">
          <thead><tr><th>Nome</th><th>Descrição</th><th>Ativo</th><th></th></tr></thead>
          <tbody>
    ...
    <div v-if="showModal" class="ag-modal-overlay" @click.self="fecharModal">
      <div class="ag-modal">
        <div class="ag-modal-header"><h5 class="mb-0">{{ form.id ? 'Editar Categoria' : 'Nova Categoria' }}</h5><button class="btn-close" @click="fecharModal" /></div>
        <div class="ag-modal-body">
          <div class="mb-3"><label class="form-label fw-bold">Nome *</label><input v-model="form.nome" type="text" class="form-control" placeholder="Nome da categoria" /></div>
          <div class="mb-3"><label class="form-label fw-bold">Descrição</label><input v-model="form.descricao" type="text" class="form-control" placeholder="Descrição opcional" /></div>
          <div class="mb-3"><div class="form-check"><input v-model="form.ativo" type="checkbox" class="form-check-input" id="cat-ativo" /><label class="form-check-label" for="cat-ativo">Ativo</label></div></div>
    ...
    <div v-if="showConfirm" class="ag-modal-overlay" @click.self="showConfirm = false">
      <div class="ag-modal" style="max-width:420px">
        ...
        <div class="ag-modal-body"><p>Excluir a categoria <strong>{{ itemParaExcluir?.nome }}</strong>?</p></div>
        <div class="ag-modal-footer">
