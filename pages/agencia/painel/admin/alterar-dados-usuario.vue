<template>
  <div>
    <div class="ag-page-header"><h1>Alterar Dados de Usuário</h1><p>Editar dados de usuário</p></div>
...
function ver(item: UsuarioAdmin) { console.log('ver', item); }
onMounted(async () => {
  agenciaStore.loadFromStorage();
  if (!agenciaStore.isAdmin) { navigateTo('/agencia/painel'); return; }
  try {
    const { data } = await api.get('/admin/usuarios/obter', authHeader());
