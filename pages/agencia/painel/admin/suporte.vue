<template>
  <div>
    <div class="ag-page-header"><h1>Suporte Admin</h1><p>Gerenciar tickets de suporte</p></div>
...
function ver(item: SuporteAdmin) { console.log('ver', item); }
onMounted(async () => {
  agenciaStore.loadFromStorage();
  if (!agenciaStore.isAdmin) { navigateTo('/agencia/painel'); return; }
  try {
    const { data } = await api.get('/admin/suporte/listar', authHeader());
