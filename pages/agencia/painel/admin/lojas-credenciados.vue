<template>
  <div>
    <div class="ag-page-header"><h1>Lojas Físicas</h1><p>Lojas físicas credenciadas</p></div>
...
function ver(item: LojaCredenciadaAdmin) { console.log('ver', item); }
onMounted(async () => {
  agenciaStore.loadFromStorage();
  if (!agenciaStore.isAdmin) { navigateTo('/agencia/painel'); return; }
  try {
    const { data } = await api.get('/admin/lojasCredenciados/listar', authHeader());
